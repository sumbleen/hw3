...
p = re.compile(r"^.+:\s+(G.+)\s+.+=\s+([.0-9]+)", re.MULTILINE)
...
#TODO: calculate the mean efficiency of all the stimulus functions and contrasts (i.e. sum the 'eff' key in the dictionary)
#try np.mean()
eff = np.mean(results['eff'])

#TODO print the mean efficiency
print(eff)
